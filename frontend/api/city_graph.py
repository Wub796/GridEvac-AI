"""Shared Houston street network for the serverless API."""
...
GRID_ROWS: int = 15
GRID_COLS: int = 15
LAT_STEP: float = 0.00135
LON_STEP: float = 0.00165
BASE_ELEV: float = 8.5
ROW_RISE: float = 0.34
COL_RISE: float = 0.24
...
def node_position(row: int, col: int) -> Tuple[float, float, float]:
    ...
    elev = BASE_ELEV + row * ROW_RISE + (GRID_COLS - 1 - col) * COL_RISE + NOISE_AMP * (math.sin(row * 1.4 + col * 0.9) + math.cos(row * 0.7 + col * 1.5)) * 0.5
...
def _road_attributes(row: int, col: int, horizontal: bool) -> Dict:
    road_name = EW_ROADS[row] if horizontal else NS_ROADS[col]
    major = row in MAJOR_EW_ROWS if horizontal else col in MAJOR_NS_COLS
    if major:
        return {"road_name": road_name, "road_class": "arterial", "lanes": 4, "speed_limit_mph": 35}
    if (row + col) % 3 == 0:
        return {"road_name": road_name, "road_class": "collector", "lanes": 3, "speed_limit_mph": 30}
    return {"road_name": road_name, "road_class": "local", "lanes": 2, "speed_limit_mph": 25}


def _node_data(row: int, col: int) -> Dict:
    lat, lon, elev = node_position(row, col)
    return {"id": node_id(row, col), "lat": lat, "lon": lon, "elevation": elev, "row": row, "col": col, "intersection_name": f"{EW_ROADS[row]} / {NS_ROADS[col]}", "district": "Downtown" if 4 <= row <= 10 and 4 <= col <= 10 else "Houston operations district"}
...
def build_graph() -> Tuple[nx.Graph, Dict[int, Dict]]:
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS - 1):
            ...
            graph.add_edge(u, v, weight=distance / (attrs["speed_limit_mph"] * 0.44704), base_weight=distance / (attrs["speed_limit_mph"] * 0.44704), distance_m=round(distance, 1), **attrs)
...
def build_substations(nodes: Dict[int, Dict]) -> List[Dict]:
    ...
    for sub in SUBSTATION_DEFS:
        center = nodes[sub["node"]]
        cr, cc = center["row"], center["col"]
        affected = [node_id(row, col) for row in range(GRID_ROWS) for col in range(GRID_COLS) if math.hypot(row - cr, col - cc) <= sub["radius"]]
        result.append({**sub, "lat": center["lat"], "lon": center["lon"], "affected_nodes": affected})
    return result
...
def build_blocks(nodes: Dict[int, Dict]) -> List[Dict]:
    ...
    for row in range(GRID_ROWS - 1):
        for col in range(GRID_COLS - 1):
            ...
            kind = "park" if is_park else kinds[(row * 3 + col * 5) % len(kinds)]
            central_bonus = 28.0 if 4 <= row <= 9 and 4 <= col <= 9 else 0.0
            height = 0.0 if is_park else 22.0 + central_bonus + float(((row * 13 + col * 17) % 8) * 8)
            blocks.append({"id": f"block-{row}-{col}", "row": row, "col": col, "lat": round(lat, 7), "lon": round(lon, 7), "kind": kind, "height_m": round(height, 1)})
    return blocks
